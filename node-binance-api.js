...
let api = function Binance(options = {}) {
    ...
    const handleFuturesSocketOpen = function (openCallback) {
        ...
        Binance.options.log('CuongPM: Futures WebSocket opened: ')
        // if (isCall === 0) {
        //     futuresTerminate(this.endpoint, true)
        // }
        // isCall = 1;
    };
    ...
    const handleFuturesSocketClose = function (reconnect, code, reason) {
        ...
        Binance.options.log('CuongPM: Futures WebSocket closed: ' + this.endpoint +
            (code ? ' (' + code + ')' : '') +
            (reason ? ' ' + reason : ''));
        // if ( Binance.options.reconnect && this.reconnect && reconnect ) {
        if (this.endpoint && parseInt(this.endpoint.length, 10) === 60) Binance.options.log('CuongPM: Futures account data WebSocket reconnecting...');
        else Binance.options.log('CuongPM: Futures WebSocket reconnecting: ' + this.endpoint + '...');
        try {
            reconnect();
        } catch (error) {
            Binance.options.log('CuongPM: Futures WebSocket reconnect error: ' + error.message);
        }
        // }
    };
    ...
    const futuresSubscribe = function (streams, callback, params = {}) {
        ...
        ws.isAlive = false;
        if (Binance.options.verbose) {
            Binance.options.log(`CuongPM: futuresSubscribe: Subscribed to [${ws.endpoint}] ${queryParams}`);
        }
        ws.on('open', handleFuturesSocketOpen.bind(ws, params.openCallback));
        ws.on('pong', handleFuturesSocketHeartbeat);
        ws.on('error', handleFuturesSocketError);
        ws.on('close', handleFuturesSocketClose.bind(ws, params));
        ws.on('message', data => {
            try {
                callback(JSON.parse(data).data);
            } catch (error) {
                Binance.options.log(`CuongPM: futuresSubscribe: Parse error: ${error.message}`);
            }
        });
        return ws;
    };
    ...
    const futuresTerminate = function (endpoint, reconnect = false) {
        Binance.options.log('CuongPM: Futures WebSocket futuresTerminate: ')
